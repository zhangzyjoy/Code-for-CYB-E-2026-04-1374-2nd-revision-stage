%% Plot results for IEEE TAES (2023) paper
%% <Aggressive Formation Tracking for Multiple Quadrotors Without Velocity Measurements Over Directed Topologies>

%% Run <init_taes_2023_att.m> to initialize the parameters and system states
%% Run <sim_taes_2023_att.slx> to load the simulation model
%% Run <plot_taes_2023_att.m> to plot the simulation results of attitude control and disturbance observation
...
%%%% Auxiliary quaternion error
%%%% Qei_tilt = [Qei_tilt_th, Qei_tilt_qx, Qei_tilt_qy, Qei_tilt_qz]'
Qei_tilt_th = zeros(NF,tLen);
Qei_tilt_qx = zeros(NF,tLen);
Qei_tilt_qy = zeros(NF,tLen);
Qei_tilt_qz = zeros(NF,tLen);
...
%%%% Auxiliary angular velocity error
wei_tilt_x = zeros(NF,tLen);
wei_tilt_y = zeros(NF,tLen);
wei_tilt_z = zeros(NF,tLen);
...
%%%% Auxiliary attitude rotation error in the LogSO(3) space
V_Phi_Rei_tilt_x = zeros(NF,tLen);
V_Phi_Rei_tilt_y = zeros(NF,tLen);
V_Phi_Rei_tilt_z = zeros(NF,tLen);
...
for tt = 1:tLen
    for iiFF = 1:NF
        ...
        Qei_tilt_th(iiFF,tt) = Qeitilt(1,iiFF,tt);
        Qei_tilt_qx(iiFF,tt) = Qeitilt(2,iiFF,tt);
        Qei_tilt_qy(iiFF,tt) = Qeitilt(3,iiFF,tt);
        Qei_tilt_qz(iiFF,tt) = Qeitilt(4,iiFF,tt);
        wei_x(iiFF,tt) = wei(1,iiFF,tt);
        wei_y(iiFF,tt) = wei(2,iiFF,tt);
        wei_z(iiFF,tt) = wei(3,iiFF,tt);
        wei_tilt_x(iiFF,tt) = weitilt(1,iiFF,tt);
        wei_tilt_y(iiFF,tt) = weitilt(2,iiFF,tt);
        wei_tilt_z(iiFF,tt) = weitilt(3,iiFF,tt);
        V_Phi_Rei_x(iiFF,tt) = V_Phi_Rei(1,iiFF,tt);
        V_Phi_Rei_y(iiFF,tt) = V_Phi_Rei(2,iiFF,tt);
        V_Phi_Rei_z(iiFF,tt) = V_Phi_Rei(3,iiFF,tt);
        V_Phi_Rei_tilt_x(iiFF,tt) = V_Phi_Reitilt(1,iiFF,tt);
        V_Phi_Rei_tilt_y(iiFF,tt) = V_Phi_Reitilt(2,iiFF,tt);
        V_Phi_Rei_tilt_z(iiFF,tt) = V_Phi_Reitilt(3,iiFF,tt);
...
%% Plot the auxiliary angular velocity tracking error
figure(2)
...
for iiFF = 1:NF
    ...
    ppk(iiFF) = plot(tout, wei_tilt_x(iiFF,:), 'color', color_vec(id_color,:), ...
                                'linestyle', linestyle_vec{id_linestyle}, 'linewidth', 1.2);
    hold on;
end
legend([ppk(1),ppk(2),ppk(3),ppk(4),ppk(5)], ...
...
box on;
xlabel('$$\it t \ (s) \rm$$','interpreter','latex');
ylabel('$$\it \tilde{\omega}_{i}^{e}(x) \rm \ (rad/s)$$','interpreter','latex');
...
for iiFF = 1:NF
    ...
    ppk(iiFF) = plot(tout, wei_tilt_y(iiFF,:), 'color', color_vec(id_color,:), ...
                                'linestyle', linestyle_vec{id_linestyle}, 'linewidth', 1.2);
    hold on;
end
box on;
xlabel('$$\it t \ (s) \rm$$','interpreter','latex');
ylabel('$$\it \tilde{\omega}_{i}^{e}(y) \rm \ (rad/s)$$','interpreter','latex');
...
for iiFF = 1:NF
    ...
    ppk(iiFF) = plot(tout, wei_tilt_z(iiFF,:), 'color', color_vec(id_color,:), ...
                                'linestyle', linestyle_vec{id_linestyle}, 'linewidth', 1.2);
    hold on;
end
box on;
xlabel('$$\it t \ (s) \rm$$','interpreter','latex');
ylabel('$$\it \tilde{\omega}_{i}^{e}(z) \rm \ (rad/s)$$','interpreter','latex');
...
box on;
xlabel('$$\it t \ (s) \rm$$','interpreter','latex');
ylabel('$$\it {\Psi}(R(Q_{i}^{e}))(x) \rm \ (rad/s)$$','interpreter','latex');
...
box on;
xlabel('$$\it t \ (s) \rm$$','interpreter','latex');
ylabel('$$\it {\Psi}(R(Q_{i}^{e}))(y) \rm \ (rad/s)$$','interpreter','latex');
...
box on;
xlabel('$$\it t \ (s) \rm$$','interpreter','latex');
ylabel('$$\it {\Psi}(R(Q_{i}^{e}))(z) \rm \ (rad/s)$$','interpreter','latex');
...
for iiFF = 1:NF
    ...
    ppk(iiFF) = plot(tout, V_Phi_Rei_tilt_x(iiFF,:), 'color', color_vec(id_color,:), ...
                                'linestyle', linestyle_vec{id_linestyle}, 'linewidth', 1.2);
    hold on;
end
legend([ppk(1),ppk(2),ppk(3),ppk(4),ppk(5)], ...
...
box on;
xlabel('$$\it t \ (s) \rm$$','interpreter','latex');
ylabel('$$\it {\Psi}(R(\tilde{Q}_{i}^{e}))(x) \rm \ (rad/s)$$','interpreter','latex');
...
for iiFF = 1:NF
    ...
    ppk(iiFF) = plot(tout, V_Phi_Rei_tilt_y(iiFF,:), 'color', color_vec(id_color,:), ...
                                'linestyle', linestyle_vec{id_linestyle}, 'linewidth', 1.2);
    hold on;
end
box on;
xlabel('$$\it t \ (s) \rm$$','interpreter','latex');
ylabel('$$\it {\Psi}(R(\tilde{Q}_{i}^{e}))(y) \rm \ (rad/s)$$','interpreter','latex');
...
for iiFF = 1:NF
    ...
    ppk(iiFF) = plot(tout, V_Phi_Rei_tilt_z(iiFF,:), 'color', color_vec(id_color,:), ...
                                'linestyle', linestyle_vec{id_linestyle}, 'linewidth', 1.2);
    hold on;
end
box on;
xlabel('$$\it t \ (s) \rm$$','interpreter','latex');
ylabel('$$\it {\Psi}(R(\tilde{Q}_{i}^{e}))(z) \rm \ (rad/s)$$','interpreter','latex');
...
%% Plot the auxiliary quaternion tracking error
figure(6)
...
for iiFF = 1:NF
    ...
    ppk(iiFF) = plot(tout, Qei_tilt_th(iiFF,:), 'color', color_vec(id_color,:), ...
                                'linestyle', linestyle_vec{id_linestyle}, 'linewidth', 1.2);
    hold on;
end
legend([ppk(1),ppk(2),ppk(3),ppk(4),ppk(5)], ...
...
box on;
xlabel('$$\it t \ (s) \rm$$','interpreter','latex');
ylabel('$$\it \tilde{\rho}_{i}^{e} \rm$$','interpreter','latex');
...
for iiFF = 1:NF
    ...
    ppk(iiFF) = plot(tout, Qei_tilt_qx(iiFF,:), 'color', color_vec(id_color,:), ...
                                'linestyle', linestyle_vec{id_linestyle}, 'linewidth', 1.2);
    hold on;
end
box on;
xlabel('$$\it t \ (s) \rm$$','interpreter','latex');
ylabel('$$\it \tilde{q}_{i}^{e}(x) \rm$$','interpreter','latex');
...
box on;
xlabel('$$\it t \ (s) \rm$$','interpreter','latex');
ylabel('$$\it \tilde{q}_{i}^{e}(y) \rm$$','interpreter','latex');
...
box on;
xlabel('$$\it t \ (s) \rm$$','interpreter','latex');
ylabel('$$\it \tilde{q}_{i}^{e}(z) \rm$$','interpreter','latex');
...
hold on;
xlabel('$$\it t \ (s) \rm$$','interpreter','latex');
ylabel('$$\it {\varpi}_{i,x}  \rm (rad/s)$$','interpreter','latex');
legend([ppk(1),ppk(2)], ...
            {'$$\it {\varpi}_{i} $$', '$$\it {\sigma}_{i}^{\varpi}$$'}, ...
             'interpreter','latex','location','best','NumColumns', 2, ...
             'box','off','color','none');
% set(gca, 'ylim', [-0.6, 0.2] .* kk_gain);
...
hold on;
xlabel('$$\it t \ (s) \rm$$','interpreter','latex');
ylabel('$$\it {\varpi}_{i,y}  \rm (rad/s)$$','interpreter','latex');
% set(gca, 'ylim', [-0.2, 0.6] .* kk_gain);
...
hold on;
xlabel('$$\it t \ (s) \rm$$','interpreter','latex');
ylabel('$$\it {\varpi}_{i,z}  \rm (rad/s)$$','interpreter','latex');
% set(gca, 'ylim', [-0.5, 0.2] .* kk_gain);
...
%% Comparison between the angular velocity tracking error and its estimation
...
legend([ppk(1),ppk(2)], ...
            {'$$\it {\sigma}_{i}^{\varpi}$$', '$$\it \hat {\sigma}_{i}^{\varpi}$$'}, ...
             'interpreter','latex','location','best', 'NumColumns', 2, ...
             'box','off','color','none');
set(gca, 'ylim', [-0.6, 0.2]);
...
hold on;
xlabel('$$\it t \ (s) \rm$$','interpreter','latex');
ylabel('$$\it {\sigma}_{i,z}^{\varpi} \rm (m/s)$$','interpreter','latex');
...
hold on;
xlabel('$$\it t \ (s) \rm$$','interpreter','latex');
ylabel('$$\it {d}_{i,x}^{\varpi} \rm (rad/s)$$','interpreter','latex');
legend([ppk(1),ppk(2)], ...
            {'$$\it {d}_{i}^{\varpi}$$', '$$\it \hat {d}_{i}^{\varpi}$$'}, ...
             'interpreter','latex','location','best','NumColumns',2, ...
             'box','off','color','none');
set(gca, 'ylim', [-0.6, 0.6] .* kk_gain);
...
hold on;
xlabel('$$\it t \ (s) \rm$$','interpreter','latex');
ylabel('$$\it {d}_{i,y}^{\varpi} \rm (rad/s)$$','interpreter','latex');
set(gca, 'ylim', [-0.8, 0.6] .* kk_gain);
...
hold on;
xlabel('$$\it t \ (s) \rm$$','interpreter','latex');
ylabel('$$\it {d}_{i,z}^{\varpi} \rm (rad/s)$$','interpreter','latex');
set(gca, 'ylim', [-0.7, 0.7] .* kk_gain);
