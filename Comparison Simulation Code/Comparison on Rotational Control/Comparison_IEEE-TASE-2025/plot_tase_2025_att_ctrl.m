%% Plot results for  IEEE TASE (2025) paper:
...
%% Run <sim_tase_2025_att_ctrl.slx> to execute the simulation model
%% Run <plot_tase_2025_att_ctrl.m> to plot the simulation results of
%% the attitude control and disturbance observer
...
err_wi_x = zeros(NF,tLen);
err_wi_y = zeros(NF,tLen);
err_wi_z = zeros(NF,tLen);

%%%% Prescribed performance envelope of the quaternion tracking error
k_1_ij_x = zeros(NF,tLen);
k_1_ij_y = zeros(NF,tLen);
k_1_ij_z = zeros(NF,tLen);

%%%% Prescribed performance envelope of the virtual angular velocity tracking error
k_2_ij_x = zeros(NF,tLen);
k_2_ij_y = zeros(NF,tLen);
k_2_ij_z = zeros(NF,tLen);

%%%% Barrier function of the quaternion tracking error
eta_1_ij_x = zeros(NF,tLen);
eta_1_ij_y = zeros(NF,tLen);
eta_1_ij_z = zeros(NF,tLen);

%%%% Barrier function of the virtual angular velocity tracking error
eta_2_ij_x = zeros(NF,tLen);
eta_2_ij_y = zeros(NF,tLen);
eta_2_ij_z = zeros(NF,tLen);
...
for tt = 1:tLen
    for iiFF = 1:NF
        ...
        err_wi_x(iiFF,tt) = err_wi(1,iiFF,tt);
        err_wi_y(iiFF,tt) = err_wi(2,iiFF,tt);
        err_wi_z(iiFF,tt) = err_wi(3,iiFF,tt);
        k_1_ij_x(iiFF,tt) = k_1_ij(1,iiFF,tt);
        k_1_ij_y(iiFF,tt) = k_1_ij(2,iiFF,tt);
        k_1_ij_z(iiFF,tt) = k_1_ij(3,iiFF,tt);
        k_2_ij_x(iiFF,tt) = k_2_ij(1,iiFF,tt);
        k_2_ij_y(iiFF,tt) = k_2_ij(2,iiFF,tt);
        k_2_ij_z(iiFF,tt) = k_2_ij(3,iiFF,tt);
        eta_1_ij_x(iiFF,tt) = eta_1_ij(1,iiFF,tt);
        eta_1_ij_y(iiFF,tt) = eta_1_ij(2,iiFF,tt);
        eta_1_ij_z(iiFF,tt) = eta_1_ij(3,iiFF,tt);
        eta_2_ij_x(iiFF,tt) = eta_2_ij(1,iiFF,tt);
        eta_2_ij_y(iiFF,tt) = eta_2_ij(2,iiFF,tt);
        eta_2_ij_z(iiFF,tt) = eta_2_ij(3,iiFF,tt);
...
subplot(222)
ppj(1) = plot(tout, k_1_ij_x(iiFF,:), ...
                    'color', 'b', 'linestyle', '-', 'linewidth', 1.2);
hold on;
ppj(2) = plot(tout, -k_1_ij_x(iiFF,:), ...
                    'color', 'b', 'linestyle', '-', 'linewidth', 1.2);
hold on;
for iiFF = 1:NF
    id_color = mod(iiFF,size(color_vec,1));
...
box on;
legend([ppk(1),ppk(2),ppk(3),ppk(4),ppk(5),ppj(1),ppj(2)], ...
            {'$$i=1$$', '$$i=2$$', '$$i=3$$', '$$i=4$$', '$$i=5$$', ...
            '$\overline{k}_{1,i}^{j}$', '$\underline{k}_{1,i}^{j}$'}, ...
             'interpreter','latex','location','best','NumColumns',7, ...
             'box','off','color','none');
box on;
xlabel('$$\it t \ (s) \rm$$','interpreter','latex');
...
subplot(223)
ppj(1) = plot(tout, k_1_ij_y(iiFF,:), ...
                    'color', 'b', 'linestyle', '-', 'linewidth', 1.2);
hold on;
ppj(2) = plot(tout, -k_1_ij_y(iiFF,:), ...
                    'color', 'b', 'linestyle', '-', 'linewidth', 1.2);
hold on;
for iiFF = 1:NF
    id_color = mod(iiFF,size(color_vec,1));
...
subplot(224)
ppj(1) = plot(tout, k_1_ij_z(iiFF,:), ...
                    'color', 'b', 'linestyle', '-', 'linewidth', 1.2);
hold on;
ppj(2) = plot(tout, -k_1_ij_z(iiFF,:), ...
                    'color', 'b', 'linestyle', '-', 'linewidth', 1.2);
hold on;
for iiFF = 1:NF
    id_color = mod(iiFF,size(color_vec,1));
...
%% Plot the virtual angular velocity tracking error
figure(4)

subplot(311)
ppj(1) = plot(tout, k_2_ij_x(iiFF,:), ...
                    'color', 'b', 'linestyle', '-', 'linewidth', 1.2);
hold on;
ppj(2) = plot(tout, -k_2_ij_x(iiFF,:), ...
                    'color', 'b', 'linestyle', '-', 'linewidth', 1.2);
hold on;
for iiFF = 1:NF
    id_color = mod(iiFF,size(color_vec,1));
    ...
    ppk(iiFF) = plot(tout, err_wi_x(iiFF,:), 'color', color_vec(id_color,:), ...
                                'linestyle', linestyle_vec{id_linestyle}, 'linewidth', 1.2);
    hold on;
end
box on;
legend([ppk(1),ppk(2),ppk(3),ppk(4),ppk(5),ppj(1),ppj(2)], ...
            {'$$i=1$$', '$$i=2$$', '$$i=3$$', '$$i=4$$', '$$i=5$$', ...
            '$\overline{k}_{2,i}^{j}$', '$\underline{k}_{2,i}^{j}$'}, ...
             'interpreter','latex','location','best','NumColumns',7, ...
             'box','off','color','none');
box on;
set(gca, 'ylim', [-2.5, 2.5]);
xlabel('$$\it t \ (s) \rm$$','interpreter','latex');
ylabel('$$\it e_{w,i}^{j}(x) \ (rad/s) \rm$$','interpreter','latex');
...
subplot(312)
ppj(1) = plot(tout, k_2_ij_y(iiFF,:), ...
                    'color', 'b', 'linestyle', '-', 'linewidth', 1.2);
hold on;
ppj(2) = plot(tout, -k_2_ij_y(iiFF,:), ...
                    'color', 'b', 'linestyle', '-', 'linewidth', 1.2);
hold on;
for iiFF = 1:NF
    id_color = mod(iiFF,size(color_vec,1));
    ...
    ppk(iiFF) = plot(tout, err_wi_y(iiFF,:), 'color', color_vec(id_color,:), ...
                                'linestyle', linestyle_vec{id_linestyle}, 'linewidth', 1.2);
    hold on;
end
box on;
...
xlabel('$$\it t \ (s) \rm$$','interpreter','latex');
ylabel('$$\it e_{w,i}^{j}(y) \ (rad/s) \rm$$','interpreter','latex');
...
subplot(313)
ppj(1) = plot(tout, k_2_ij_z(iiFF,:), ...
                    'color', 'b', 'linestyle', '-', 'linewidth', 1.2);
hold on;
ppj(2) = plot(tout, -k_2_ij_z(iiFF,:), ...
                    'color', 'b', 'linestyle', '-', 'linewidth', 1.2);
hold on;
for iiFF = 1:NF
    id_color = mod(iiFF,size(color_vec,1));
    ...
    ppk(iiFF) = plot(tout, err_wi_z(iiFF,:), 'color', color_vec(id_color,:), ...
                                'linestyle', linestyle_vec{id_linestyle}, 'linewidth', 1.2);
    hold on;
end
box on;
set(gca, 'ylim', [-5.0, 5.0]);
xlabel('$$\it t \ (s) \rm$$','interpreter','latex');
ylabel('$$\it e_{w,i}^{j}(z) \ (rad/s) \rm$$','interpreter','latex');
