%% The attitude controller is adopted from the IEEE-TCST (2024) paper:
...
%% The corresponding simulation model is <sim_tcst_2024_att_ctrl.slx>
...
%% Run <sim_tcst_2024_att_ctrl.slx> to execute the attitude control simulation
%% Run <plot_tcst_2024_att_ctrl.m> to plot the simulation results
...
for iuav = 1:NF
    ...
    err_Ri_bar_init(:,iuav) = wei_init(:,iuav) + kr1 * V_Phi_Rei_init(:,iuav);
    Fi_init(:,iuav) = cal_askew_mat( wei_init(:,iuav) ) * ( Rei_init(:,:,iuav)' ) * wci_init(:,iuav) ...
                                - ( Rei_init(:,:,iuav)' ) * dwci_init(:,iuav);
end
