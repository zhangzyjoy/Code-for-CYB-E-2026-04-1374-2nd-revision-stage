%% The attitude controller is adopted from the IEEE-TCYB (2024) paper:
%% <Adaptive Formation Tracking Control of Multiple
%% Vertical Takeoff and Landing UAVs With Bearing-Only Measurements>

%% Considering external disturbances, a fixed-time convergent disturbance observer is designed for disturbance compensation
%% The corresponding simulation model is <sim_tcyb_2024_att_ctrl.slx>
%% The model updates wi, Qi, Qei, and V_Phi_Rei through differential equations

%% Run <init_tcyb_2024_att_ctrl.m> to initialize the parameters and system states
%% Run <sim_tcyb_2024_att_ctrl.slx> to execute the attitude control simulation
%% Run <plot_tcyb_2024_att_ctrl.m> to plot the simulation results
...
%% Parameter settings for attitude controller in Rotational channel

l1 = diag([5.0, 5.0, 4.0]);
l2 = diag([5.0, 5.0, 4.0]);
mu = diag([10.0, 10.0, 10.0, 8.0, 8.0, 8.0]);
kw = diag([10.0, 10.0, 8.0]);
ksg = diag([10.0, 10.0, 8.0]);
...
ui_init = zeros(3,NF);
for iuav = 1:NF
    ui_init(:,iuav) = -g0 * e3 + dv0_init(:);
end

%%%% Initialize the desired quaternion states
Qci_init = repmat([1;0;0;0],1,NF);

%%%% Initialize the Rodrigues parameter
sgi_init = zeros(3,NF);
sgci_init = zeros(3,NF);
...
for iuav = 1:NF
    sgi_init(:, iuav) = cal_Rodrig_from_Q( Qi_init(:,iuav) );
    sgci_init(:, iuav) = cal_Rodrig_from_Q( Qci_init(:,iuav) );
    Ri_init(:, :, iuav) = cal_R_with_Rodrig( sgi_init(:, iuav) );
    Rci_init(:, :, iuav) = cal_R_with_Rodrig( sgci_init(:, iuav) );
end

%%%% Initialization of the commanded angular velocity
wci_init = zeros(3,NF);
dwci_init = zeros(3,NF);
for iuav = 1:NF
...
sgei_init = zeros(3,NF);
Qei_init = zeros(4,NF);
Rei_init = zeros(3,3,NF);
wei_init = zeros(3,NF);
V_Phi_Rei_init = zeros(3,NF);
for iuav = 1:NF
    sgei_init(:,iuav) = cal_Rodrig_err(sgi_init(:,iuav), sgci_init(:,iuav));
    Qei_init(:,iuav) = cal_Q_mul_Q(cal_inv_Q(Qci_init(:,iuav)), Qi_init(:,iuav));
    Rei_init(:,:,iuav) = cal_R_with_Rodrig(sgei_init(:,iuav));
