%% Leader-follower formation control -- Translational Control
%% Method proposed in the Manuscript : 
%% Distributed Practical Fixed-Time Distributed Formation Controller (PFxTDFC)

%% Distributed observer for estimating the desired positions and velocities
%% of followers + disturbance observer + translational controller

%% Revised version with a new leader reference trajectory and
%% a time-varying desired formation configuration
...
%% Initialization of state variables in the translational channel
...
d_chi_init = zeros(3,NF);
kk_chi = alfa_theta;

for iiuav = 1:NF
    theta_chi_1 = ( beta1 * cal_frac_squ(abs(ksii_0(:,iiuav)), p_dbeta1, q_dbeta1) .* sign(ksii_0(:,iiuav)) .* ( -1 + 2 ./ ( 1 + exp( -kk_chi * ksii_0(:,iiuav) ) ) ) ...
                                + cal_frac_squ(abs(ksii_0(:,iiuav)), p_beta1, q_beta1) .* ( 2 .* kk_chi .* exp(-kk_chi .* ksii_0(:,iiuav)) ) ./ ( ( 1 + exp(-kk_chi .* ksii_0(:,iiuav)) ) .^ 2 ) ) ...
                                .* etai_0(:,iiuav);
    theta_chi_2 = ( beta2 * cal_frac_squ(abs(ksii_0(:,iiuav)), p_dbeta2, q_dbeta2) .* sign(ksii_0(:,iiuav)) .* ( -1 + 2 ./ ( 1 + exp( -kk_chi * ksii_0(:,iiuav) ) ) ) ...
                                + cal_frac_squ(abs(ksii_0(:,iiuav)), p_beta2, q_beta2) .* ( 2 .* kk_chi .* exp(-kk_chi .* ksii_0(:,iiuav)) ) ./ ( ( 1 + exp(-kk_chi .* ksii_0(:,iiuav)) ) .^ 2 ) ) ...
                                .* etai_0(:,iiuav);
    d_chi_init(:,iiuav) = -kai_chi_1 * theta_chi_1(:) - kai_chi_2 * theta_chi_2(:);
    ui_init(:,iiuav) = g0 * e3 + d_chi_init(:,iiuav);
end
