...
To maintain the upper bound for a lower settling time for the rotational disturbance observation, the tuning principles for the observer parameters should be set as $\alpha _1 ^{ \varpi } \uparrow$, $\alpha _2 ^{ \varpi } \downarrow$, $\mu _{ d } ^{ \varpi } \uparrow$, $h _{ i } ^{ \varpi, 1 } \uparrow$, $h _{ i } ^{ \varpi, 2 } \uparrow$, and $h _{ i } ^{ \varpi, 3 }$ is tuned freely.
